...
CONFIG_SCRIPT="$RESEARCH_DIR/configs/edge_device_pim.py"
BINARY="$RESEARCH_DIR/benchmarks/pim_functional/main"
BENCHMARK_BINARY="$RESEARCH_DIR/benchmarks/pim_functional/main_benchmark"

# Konfiguration der Parameter-Sweeps
SIZES=(1 2 4 8 16 32 64 128 256 512 1024 2048 4096 8192) # Gewünschte Vektorlängen
# SIZES=($((2**14)) $((2**16)) $((2**18)) $((2**20)) $((2**22)))
# ELEMENT_SIZE=8              # 8 Byte entspricht sizeof(double)
ELEMENT_SIZE=4              # 8 Byte entspricht sizeof(FP16)
# ELEMENT_SIZE=2              # 8 Byte entspricht sizeof(FP16)

DEBUG_FLAG="PIM"

echo "Starte parallele gem5 PIM-Simulationen..."
...
for N in "${SIZES[@]}"; do
  ...
  OUT_DIR="$RESULTS_DIR/pim_$N"
  OUT_DIR_BENCHMARK="$RESULTS_DIR/pim_${N}_benchmark"
...
RESULT_CSV="$RESULTS_DIR/pim_performance_study.csv"
TEMP_CSV="$RESULTS_DIR/pim_performance_study_temp.csv"
...
for N in "${SIZES[@]}"; do
  STATS="$RESULTS_DIR/pim_$N/stats.txt"
  BENCH_STATS="$RESULTS_DIR/pim_benchmark/stats.txt"
