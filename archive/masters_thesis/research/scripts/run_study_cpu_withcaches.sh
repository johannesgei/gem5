...
CONFIG_SCRIPT="$RESEARCH_DIR/configs/edge_device_cpu_withcaches.py"
# BINARY="$RESEARCH_DIR/benchmarks/cpu/main_double"
# BENCHMARK_BINARY="$RESEARCH_DIR/benchmarks/cpu/main_double_benchmark"
BINARY="$RESEARCH_DIR/benchmarks/cpu/main_double_uncached"
BENCHMARK_BINARY="$RESEARCH_DIR/benchmarks/cpu/main_double_uncached_benchmark"
...
# SIZES=(1 2 4 8 16 32 64 128 256 512 1024 2048 4096 8192 16384) # Gewünschte Vektorlängen
...
echo "Starte parallele gem5 CPU-Simulationen (With Caches)..."
...
for N in "${SIZES[@]}"; do
  ...
  OUT_DIR="$RESULTS_DIR/cpu_withcaches_$N"
  OUT_DIR_BENCHMARK="$RESULTS_DIR/cpu_withcaches_${N}_benchmark"
...
RESULT_CSV="$RESULTS_DIR/cpu_withcaches_performance_study.csv"
TEMP_CSV="$RESULTS_DIR/cpu_withcaches_performance_study_temp.csv"
...
for N in "${SIZES[@]}"; do
  STATS="$RESULTS_DIR/cpu_withcaches_$N/stats.txt"
  BENCH_STATS="$RESULTS_DIR/cpu_withcaches_${N}_benchmark/stats.txt"
